...
def max_latency_strict(row: pd.Series):
    """Returns the max latency in a frame."""
...
def plot_metric_across_folder(folder_path: str, metric_func, framerate: float = 60.0, ymax: float = None):
    ...
    max_of_metrics = combined_metrics.max(axis=1) # for max max latency
    
    # Plotting
    time_axis = common_time - common_time[0]
    
    plt.figure(figsize=(12, 6))
    plt.plot(time_axis, max_of_metrics, label=f"Average of {metric_func.__name__}", color='blue', linewidth=2)
    plt.xlabel("Time (seconds)")
    ...
    plt.tight_layout()
    plt.show()
    print("Average max latency (graph):", np.nanmean(mean_of_metrics))


if __name__ == "__main__":
    # Plot a single client's raw data
    ...
    # Plot the average max latency over all clients in the simulation
