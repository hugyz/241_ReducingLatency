...
```
--- B (Edge) ---
Listening on: 192.168.1.5:<edge_port>
Target: 192.168.1.5:8000
```

Use the printed `<edge_port>` when connecting clients to this edge.
...
# Terminal 2
python arena_game.py --client-id p1 --edge <host>:<port> --region A --color 0

# Terminal 3
python arena_game.py --client-id p2 --edge <host>:<port> --region A --color 1
```
...
# Prints: Listening on: <host>:<edge_port>

# Terminal 3 — Player 1 (region A, direct to main)
python arena_game.py --client-id p1 --edge <host>:<main_port> --region A --color 0

# Terminal 4 — Player 2 (region B, through edge)
python arena_game.py --client-id p2 --edge <host>:<edge_port> --region B --color 1
```
...
# Sydney players
python arena_game.py --client-id sydney1 --edge <host>:<main_port> --region Sydney --color 0
python arena_game.py --client-id sydney2 --edge <host>:<main_port> --region Sydney --color 1

# Perth players
python arena_game.py --client-id perth1 --edge <host>:<edge_port> --region Perth --color 2
python arena_game.py --client-id perth2 --edge <host>:<edge_port> --region Perth --color 3
```
...
`run_clients.sh` is a lightweight script for load testing and repeatable latency experiments. Unlike `automation.sh`, it does not manage servers — you must start the main and edge servers manually first. It then spawns a configurable number of clients against a fixed edge address.

Set the `EDGE` variable inside `run_clients.sh` to point at the server you want to test against:
